...
# WPS variables
WPS_DISABLE         = True # Flag to skip WPS scan and attacks#######################################################################################
		...
		while index < len(targets):
			...
			for already in CRACKED_TARGETS:
				if already.bssid == targets[index].bssid:
					...
					ri = 'y'##################################################################################raw_input(GR+' [+] '+W+'do you want to crack this access point again? ('+G+'y/'+O+'n'+W+'): ')
					if ri.lower() == 'n':
						targets.pop(index)
						index -= 1
					break
			...
			if os.path.exists(handshake_file):
				...
				while ri != 's' and ri != 'c' and ri != 'o': 
					ri = 'o'######################################################################################raw_input(GR+' [+] '+W+'enter '+G+'s'+W+', '+O+'c,'+W+' or '+R+'o'+W+': '+G).lower()
				print W+"\b",
				if ri == 's': 
					targets.pop(index)
					index -= 1
